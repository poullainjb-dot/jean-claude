...
export function PriceImportForm() {
  ...
  const [stats, setStats] = useState<ImportPricesStats | null>(null);
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md">
      <label className="flex flex-col gap-1">
        <span className="text-sm font-medium">Prices CSV</span>
        <input
          type="file"
          accept=".csv"
      ...
      {stats && (
        <div className="text-sm border rounded p-3 border-green-600/30 bg-green-600/10">
          ...
          <p>Inserted {stats.inserted} new price(s)</p>
          <p>Updated {stats.updated} existing price(s)</p>
          <p>Unchanged {stats.unchanged} price(s)</p>
        </div>
      )}
